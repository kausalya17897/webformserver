...
const service = {
  ...
  async addEmployee(request, response) {
    try {
      const { insertedId: _id } = await db.employee.insertOne(request.body);
  ...
  async updateEmployee(request, response) {
    try {
      ...
      //validation indb
      const { value } = await db.employee.findOneAndUpdate(
  ...
  async deleteEmployee(request, response) {
    try {
      await db.employee.deleteOne({ _id: ObjectId(request.params.id) });
